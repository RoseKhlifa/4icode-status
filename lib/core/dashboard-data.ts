 ...
 *
 * 4i.codes 版本:
 *   - 移除官方状态爬取 (openai/anthropic status page)
 *   - 移除 group_info 表 (改由 config 里的 groupName 直接呈现)
 *   - 移除主从选举
 */
...
async function loadDashboardDataInternal(options?: {
  ...
  const loadData = async (): Promise<DashboardLoadResult> => {
    ...
    // groupInfos 从 provider config 里派生: 每个不重复 groupName 一条
    const groupInfoSummaries: GroupInfoSummary[] = [];
    const seenGroups = new Set<string>();
    for (const cfg of allConfigs) {
      const gn = (cfg.groupName ?? "").trim();
      if (!gn || seenGroups.has(gn)) continue;
      seenGroups.add(gn);
      groupInfoSummaries.push({
        groupName: gn,
        websiteUrl: null,
        tags: "",
      });
